...
def producer(data: torch.Tensor, slicers, queue: Queue):
    """Produces slices of data for processing."""
    try:
        for sl in slicers:
            workon = data[sl]
            queue.put((workon, sl))
...
def mc_forward_passes(model, x, num_passes=20):
    ...
    enable_mc_dropout(model)  # Enable dropout layers
    preds = []
...
def save_uncertainty_map(uncertainty: torch.Tensor, input_image_path: str, output_dir: str):
    import os
    ...
    nii = nib.load(input_image_path)
    affine = nii.affine
    header = nii.header
    reference_shape = nii.shape

    uncertainty_np = uncertainty.cpu().numpy()
    uncertainty_np = np.squeeze(uncertainty_np).astype(np.float32)

    if uncertainty_np.shape != reference_shape:
        print(f"Resampling uncertainty map from {uncertainty_np.shape} to {reference_shape}...")
        # Resample using SimpleITK
        unc_img = sitk.GetImageFromArray(uncertainty_np)
        ref_img = sitk.ReadImage(input_image_path)
        resampler = sitk.ResampleImageFilter()
        resampler.SetReferenceImage(ref_img)
        resampler.SetInterpolator(sitk.sitkLinear)
        unc_img_resampled = resampler.Execute(unc_img)
        uncertainty_np = sitk.GetArrayFromImage(unc_img_resampled)
...
def resample_to_reference(uncertainty_np, reference_path):
    ...
    # Convert uncertainty to SimpleITK image
    unc_img = sitk.GetImageFromArray(uncertainty_np.astype(np.float32))
    unc_img.SetSpacing(ref_spacing)
    unc_img.SetOrigin(ref_origin)
    unc_img.SetDirection(ref_direction)
    ...
    unc_resampled_np = sitk.GetArrayFromImage(unc_img_resampled)
    return unc_resampled_np
...
class nnUNetPredictor(object):
    ...
    def _internal_predict_sliding_window_return_logits(self, data: torch.Tensor, slicers, do_on_device: bool = True):
        ...
        try:
            ...
            uncertainty_map = torch.zeros(data.shape[1:], dtype=torch.float32, device=results_device)
            n_uncertainty = torch.zeros(data.shape[1:], dtype=torch.float32, device=results_device)
            ...
            with tqdm(desc=None, total=len(slicers), disable=not self.allow_tqdm) as pbar:
                while True:
                    ...
                    mc_preds = mc_forward_passes(self.network, workon, num_passes=20)
                    prediction = mc_preds.mean(dim=0)[0].to(results_device)
                    patch_uncertainty = mc_preds.var(dim=0)[0].to(results_device)

                    if self.use_gaussian:
                        prediction *= gaussian
                        patch_uncertainty *= gaussian
                    
                    predicted_logits[sl] += prediction
                    uncertainty_map[sl[1:]] += patch_uncertainty
                    n_predictions[sl[1:]] += gaussian
                    n_uncertainty[sl[1:]] += gaussian
            ...
            torch.div(uncertainty_map, n_uncertainty, out=uncertainty_map)
            # check for infs
            ...
            raise e
        return predicted_logits, uncertainty_map
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor) \
        ...
        with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            if self.perform_everything_on_device and self.device != 'cpu':
                try:
                    predicted_logits, uncertainty_map = self._internal_predict_sliding_window_return_logits(
            ...
            uncertainty_map = uncertainty_map[slicer_revert_padding[1:]]
    ...
    def predict_from_files_sequential(self,
        ...
        for li, of, sps in zip(list_of_lists_or_source_folder, output_filename_truncated, seg_from_prev_stage_files):
            ...
            if of is not None:
                ...
                print(f"✅ Saved segmentation prediction to: {of}{self.dataset_json['file_ending']}")
            else:
...
def set_dropout_probability(model, p=0.05):
...
if __name__ == '__main__':
    ...
    # Set dropout probability dynamically
    set_dropout_probability(predictor.network, p=0.05)
    ...
    if predictor.latest_uncertainty_map is not None:
        print("Uncertainty map shape:", predictor.latest_uncertainty_map.shape)
        print("Contains NaN:", torch.isnan(predictor.latest_uncertainty_map).any().item())
        print("Min value:", torch.min(predictor.latest_uncertainty_map[~torch.isnan(predictor.latest_uncertainty_map)]).item() if torch.sum(~torch.isnan(predictor.latest_uncertainty_map)) > 0 else "all nan")
        print("Max value:", torch.max(predictor.latest_uncertainty_map[~torch.isnan(predictor.latest_uncertainty_map)]).item() if torch.sum(~torch.isnan(predictor.latest_uncertainty_map)) > 0 else "all nan")
    
        save_uncertainty_map(
            predictor.latest_uncertainty_map,
            input_image_path = "/scratch/hpl14/nnUNet_raw/Dataset999_BraTSPED/imagesTs/BraTS-PED-00062-0000.nii.gz",
...
for m in predictor.network.modules():
    if isinstance(m, torch.nn.Dropout):
        print(f"Dropout layer: {m}, Probability: {m.p}")

print("Contains NaN:", torch.isnan(predictor.latest_uncertainty_map).any().item())

if predictor.latest_uncertainty_map is None:
    print("Error: Uncertainty map not generated!")
    return
