...
def save_uncertainty_map_remap(uncertainty: np.ndarray, properties: dict, plans_manager, configuration_manager, output_path: str, label_manager=None):
    """
    Save uncertainty map using the SAME pipeline as export_prediction_from_logits, 
    but keep values continuous (no argmax/segmentation conversion).
    
    This ensures uncertainty maps are properly:
    1. Resampled from processing space to original spacing
    2. Uncropped (inserted back into original bbox)
    3. Transposed back to original orientation
    4. Saved with original affine matrix
    """
    import numpy as np
    import torch
    from acvl_utils.cropping_and_padding.bounding_boxes import insert_crop_into_image
    
    print(f"[DEBUG] Starting uncertainty map saving pipeline")
    print(f"[DEBUG] Input uncertainty shape: {uncertainty.shape}, dtype: {uncertainty.dtype}")
    
    # Convert to torch tensor if needed for resampling
    if isinstance(uncertainty, np.ndarray):
        uncertainty = torch.from_numpy(uncertainty)
    
    # Ensure we have shape (1, H, W, D) for single-channel uncertainty
    if uncertainty.ndim == 3:
        uncertainty = uncertainty[None, ...]
        print(f"[DEBUG] Added channel dimension, new shape: {uncertainty.shape}")
    elif uncertainty.ndim == 4 and uncertainty.shape[0] != 1:
        # If multiple channels, average them
        uncertainty = torch.mean(uncertainty, dim=0, keepdim=True)
        print(f"[DEBUG] Averaged {uncertainty.shape[0]} channels, new shape: {uncertainty.shape}")

    
    # ===== STEP 1: RESAMPLE from processing space to original spacing =====
    # This is the SAME as convert_predicted_logits_to_segmentation_with_correct_shape
    ...
    print(f"[DEBUG] Step 1: Resampling")
    print(f"  - Target shape: {properties['shape_after_cropping_and_before_resampling']}")
    print(f"  - Current spacing: {current_spacing}")
    print(f"  - Target spacing: {[properties['spacing'][i] for i in plans_manager.transpose_forward]}")
    ...
    # Remove channel dimension (uncertainty is single channel)
    if uncertainty_resampled.shape[0] == 1:
        uncertainty_resampled = uncertainty_resampled[0]
    
    # ===== STEP 2: UNCROP - insert back into original bbox =====
    print(f"[DEBUG] Step 2: Uncropping")
    print(f"  - Shape before cropping: {properties['shape_before_cropping']}")
    print(f"  - Crop bbox: {properties['bbox_used_for_cropping']}")
    
    uncertainty_uncropped = np.zeros(properties['shape_before_cropping'], dtype=np.float32)
    uncertainty_uncropped = insert_crop_into_image(uncertainty_uncropped, uncertainty_resampled, 
    ...
    print(f"[DEBUG] Step 3: Transposing back")
    print(f"  - Transpose backward: {plans_manager.transpose_backward}")
    
    uncertainty_final = uncertainty_uncropped.transpose(plans_manager.transpose_backward)
    print(f"[DEBUG] After transpose_backward: {uncertainty_final.shape}")
    
    # CRITICAL FIX: Check if we need additional transpose to match original shape
    # The shape_before_cropping is what the original image dimensions should be
    expected_shape = properties['shape_before_cropping']
    print(f"[DEBUG] Expected shape (shape_before_cropping): {expected_shape}")
    print(f"[DEBUG] Current shape: {uncertainty_final.shape}")
    
    # If shapes don't match, try to find the correct permutation
    if uncertainty_final.shape != expected_shape:
            # Common case: (155, 240, 240) needs to become (240, 240, 155)
            # This requires transpose (1, 2, 0) to move first axis to last
            if (uncertainty_final.shape[0] == expected_shape[2] and 
                uncertainty_final.shape[1] == expected_shape[0] and 
                uncertainty_final.shape[2] == expected_shape[1]):
                uncertainty_final = uncertainty_final.transpose((1, 2, 0))
                print(f"[DEBUG] Applied additional transpose (1, 2, 0), new shape: {uncertainty_final.shape}")
            elif (uncertainty_final.shape[0] == expected_shape[2] and 
                  uncertainty_final.shape[1] == expected_shape[1] and 
                  uncertainty_final.shape[2] == expected_shape[0]):
                uncertainty_final = uncertainty_final.transpose((2, 1, 0))
                print(f"[DEBUG] Applied additional transpose (2, 1, 0), new shape: {uncertainty_final.shape}")
    
    print(f"[DEBUG] Final shape before saving: {uncertainty_final.shape}")
    
    # ===== STEP 4: SAVE with proper affine (using image_reader_writer) =====
    print(f"[DEBUG] Step 4: Saving with proper affine")
    
    # Use the same writer class as nnU-Net uses for segmentations
    rw = plans_manager.image_reader_writer_class()
    
    # The write_seg method expects uint8/uint16, but we need float32 for uncertainty
    # So we'll manually save using nibabel with the same affine handling
    
    # For NibabelIO and NibabelIOWithReorient, the affine is stored in properties['nibabel_stuff']
    if 'nibabel_stuff' in properties:
        import nibabel as nib
        # Apply the same transpose that write_seg does: (2, 1, 0)
        uncertainty_for_save = uncertainty_final.transpose((2, 1, 0)).astype(np.float32)
        
        if 'reoriented_affine' in properties['nibabel_stuff']:
            # This is NibabelIOWithReorient - need to reorient back
            from nibabel.orientations import io_orientation, axcodes2ornt, ornt_transform
            
            affine = properties['nibabel_stuff']['reoriented_affine']
            uncertainty_nib = nib.Nifti1Image(uncertainty_for_save, affine=affine)
            ...
            uncertainty_nib_reoriented = uncertainty_nib.as_reoriented(from_canonical)
            
            print(f"  - Using NibabelIOWithReorient")
            print(f"  - Original affine:\n{properties['nibabel_stuff']['original_affine']}")
            print(f"  - Reoriented affine:\n{uncertainty_nib_reoriented.affine}")
            
            nib.save(uncertainty_nib_reoriented, output_path)
        else:
            # This is standard NibabelIO
            affine = properties['nibabel_stuff']['original_affine']
            uncertainty_nib = nib.Nifti1Image(uncertainty_for_save, affine=affine)
            
            print(f"  - Using NibabelIO")
            print(f"  - Original affine:\n{affine}")
            
            nib.save(uncertainty_nib, output_path)
    else:
        # Fallback: use SimpleITK or other reader/writer
        # For now, just save with identity affine
        import nibabel as nib
        uncertainty_for_save = uncertainty_final.astype(np.float32)
        affine = np.eye(4)
        if 'spacing' in properties:
            affine[0, 0] = properties['spacing'][0]
            affine[1, 1] = properties['spacing'][1]
            affine[2, 2] = properties['spacing'][2]
        
        print(f"  - Using fallback saving with spacing-based affine")
        print(f"  - Affine:\n{affine}")
    ...
    print(f"[DEBUG] Final shape: {uncertainty_final.shape}")                                                                                    
...
class nnUNetPredictor(object):
    ...
    def predict_from_files_sequential(self,
        ...
        for li, of, sps in zip(list_of_lists_or_source_folder, output_filename_truncated, seg_from_prev_stage_files):
            ...
            # CRITICAL FIX: Add the original image path to properties for affine/header loading
            # Use the first image file (typically _0000.nii.gz for channel 0/FLAIR in BraTS)
            if 'path' not in data_properties_for_export and li is not None and len(li) > 0:
                data_properties_for_export['path'] = li[0]  # First modality file
                print(f"[DEBUG] Added original image path to properties: {li[0]}")
            ...
            if self.latest_uncertainty_map is not None:
                ...
                if of is not None:
                    # Use case-specific uncertainty filename instead of hardcoded "uncertainty.nii.gz"
                    unc_output_path = of + "_uncertainty" + self.dataset_json['file_ending']
                else:
                    unc_output_path = "uncertainty.nii.gz"
                unc_map = self.latest_uncertainty_map.cpu().numpy()
                # Ensure 4D shape (C, X, Y, Z)
                if unc_map.ndim == 3:
                    unc_map = unc_map[None, ...]
                elif unc_map.ndim != 4:
                    raise ValueError(f"Uncertainty map must be 4D (C, X, Y, Z), got shape {unc_map.shape}")
...
if __name__ == '__main__':
    # Use the CLI entry point to handle command-line arguments
