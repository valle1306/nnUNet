...
def save_uncertainty_map(uncertainty: torch.Tensor, input_image_path: str, output_dir: str):
    """
    Save uncertainty map with proper shape handling
    """
    import os
    import nibabel as nib
    os.makedirs(output_dir, exist_ok=True)
    
    # Load reference image for affine and header
    nii = nib.load(input_image_path)
    affine = nii.affine
    header = nii.header
    reference_shape = nii.shape
    
    # Convert uncertainty to numpy and handle dimensions properly
    uncertainty_np = uncertainty.cpu().numpy()
    
    # Remove batch dimension if present (should be first dimension)
    if uncertainty_np.ndim == 4 and uncertainty_np.shape[0] == 1:
        uncertainty_np = uncertainty_np[0]  # Remove batch dimension
    
    # If we have multiple channels, compute mean across channels
    if uncertainty_np.ndim == 4:  # (C, H, W, D)
        uncertainty_np = np.mean(uncertainty_np, axis=0)  # Mean across channels
    
    # Ensure we have 3D uncertainty map
    uncertainty_np = np.squeeze(uncertainty_np).astype(np.float32)
    
    # Debug info
    print(f"Reference image shape: {reference_shape}")
    print(f"Uncertainty map shape: {uncertainty_np.shape}")
    
    if uncertainty_np.shape != reference_shape:
        print(f"WARNING: Shape mismatch! Reference: {reference_shape}, Uncertainty: {uncertainty_np.shape}")
    ...
    try:
        ...
        print(f"❌ Error saving uncertainty map: {e}")
        print(f"Uncertainty shape: {uncertainty_np.shape}, Reference shape: {reference_shape}")
