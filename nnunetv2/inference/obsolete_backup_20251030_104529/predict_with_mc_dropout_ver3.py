...
def enable_mc_dropout(model):
    ...
    for name, m in model.named_modules():
        if m.__class__.__name__.startswith('Dropout'):
            if getattr(m, 'p', None) != 0.05:
                incorrect.append((name, m.p))
    if incorrect:
        print(f"WARNING: The following Dropout layers do not have p=0.05: {incorrect}")
    else:
        print("All Dropout layers set to p=0.05 and enabled for MC Dropout.")


def save_uncertainty_map_with_correct_shape(uncertainty_map, properties_dict, 
                                          plans_manager, configuration_manager, 
                                          output_path):
    """
    Apply the same postprocessing pipeline as predictions
    Uses nnU-Net's internal functions to ensure perfect spatial correspondence
    """
    import torch
    import nibabel as nib
    import numpy as np
    from nnunetv2.utilities.label_handling.label_handling import bounding_box_to_slice
    
    print(f"[DEBUG] Starting uncertainty postprocessing")
    print(f"[DEBUG] Input uncertainty shape: {uncertainty_map.shape}, dtype: {uncertainty_map.dtype}")
    
    # Ensure uncertainty map has same format as logits (4D: C, H, W, D)
    if uncertainty_map.ndim == 3:
        uncertainty_map = uncertainty_map[None, ...]  # Add channel dimension
        print(f"[DEBUG] Added channel dimension: {uncertainty_map.shape}")
    
    # Convert to torch tensor (like logits)
    if isinstance(uncertainty_map, np.ndarray):
        uncertainty_map = torch.from_numpy(uncertainty_map).float()
    
    print(f"[DEBUG] Properties available: {list(properties_dict.keys())}")
    
    # STEP 1: Resample back to cropped shape (EXACT same as predictions)
    ...
    print(f"[DEBUG] Resampling from {uncertainty_map.shape[1:]} to {properties_dict['shape_after_cropping_and_before_resampling']}")
    print(f"[DEBUG] Current spacing: {current_spacing}")
    print(f"[DEBUG] Target spacing: {spacing_transposed}")
    
    uncertainty_resampled = configuration_manager.resampling_fn_probabilities(
        uncertainty_map,
    ...
    if isinstance(uncertainty_resampled, torch.Tensor):
        uncertainty_resampled = uncertainty_resampled.cpu().numpy()
    
    # Remove channel dimension if single channel
    if uncertainty_resampled.shape[0] == 1:
        uncertainty_resampled = uncertainty_resampled[0]
    
    print(f"[DEBUG] After resampling: {uncertainty_resampled.shape}")
    
    # STEP 2: Uncrop - insert back into original image space (same as predictions)
    uncertainty_uncropped = np.zeros(properties_dict['shape_before_cropping'], 
                                   dtype=uncertainty_resampled.dtype)
    
    # Use nnU-Net's exact bounding box logic
    slicer = bounding_box_to_slice(properties_dict['bbox_used_for_cropping'])
    uncertainty_uncropped[slicer] = uncertainty_resampled
    
    print(f"[DEBUG] After uncropping: {uncertainty_uncropped.shape}")
    
    # STEP 3: Revert transpose to original orientation (same as predictions)
    uncertainty_final = uncertainty_uncropped.transpose(plans_manager.transpose_backward)
    
    print(f"[DEBUG] After transpose (final): {uncertainty_final.shape}")
    
    # STEP 4: Save with original image's affine matrix
    if 'path' in properties_dict and properties_dict['path']:
        try:
            # Load the first modality's file for affine reference
            original_path = properties_dict['path']
            if isinstance(original_path, list):
                original_path = original_path[0]  # Use first modality
            
            print(f"[DEBUG] Loading affine from: {original_path}")
            ...
            print(f"[DEBUG] Final uncertainty shape: {uncertainty_final.shape}")
            
            if uncertainty_final.shape != original_shape:
                print(f"[WARNING] Shape mismatch! Original: {original_shape}, Uncertainty: {uncertainty_final.shape}")
                
                # Try to find the correct transpose to match the original image shape
                if len(uncertainty_final.shape) == 3 and len(original_shape) == 3:
                    # Try different transpose permutations to match the original shape
                    permutations = [(0,1,2), (0,2,1), (1,0,2), (1,2,0), (2,0,1), (2,1,0)]
                    
                    for i, perm in enumerate(permutations):
                        transposed = uncertainty_final.transpose(perm)
                        if transposed.shape == original_shape:
                            uncertainty_final = transposed
                            print(f"[SUCCESS] ✅ Applied transpose {perm} to match original shape!")
                            print(f"[DEBUG] New uncertainty shape: {uncertainty_final.shape}")
                            break
                    else:
                        print("[ERROR] Could not find a transpose permutation to match original shape")
                        print("[INFO] Saving uncertainty with current shape - may need manual orientation correction")
            else:
                print(f"[SUCCESS] ✅ Shapes match perfectly!")
                
        except Exception as e:
            print(f"[ERROR] Could not load original image: {e}")
            affine = np.eye(4)
            header = None
    else:
        print(f"[WARNING] No original image path in properties")
        affine = np.eye(4) 
        header = None
    
    # Save uncertainty map
    uncertainty_nii = nib.Nifti1Image(uncertainty_final.astype(np.float32), affine, header)
    nib.save(uncertainty_nii, output_path)
    
    print(f"✅ Saved uncertainty map: {output_path}")
    print(f"   Final shape: {uncertainty_final.shape}")
    print(f"   Data range: [{uncertainty_final.min():.6f}, {uncertainty_final.max():.6f}]")
...
class nnUNetPredictor(object):
    ...
    def predict_from_data_iterator(self,
                                   ...
                                   uncertainty_output_folder: str = None):
        """
        MC Dropout inference with per-case uncertainty export. 
        IMPORTANT: This method uses the existing sliding window pipeline but enables MC dropout
        """
        results = []
        if save_uncertainty and uncertainty_output_folder is not None:
            maybe_mkdir_p(uncertainty_output_folder)
            
        # Enable MC dropout for all iterations
        ...
        for preprocessed in data_iterator:
            ...
            if ofile is not None:
                print(f'\nPredicting {os.path.basename(ofile)} with MC Dropout:')
            else:
                print(f'\nPredicting image of shape {data.shape} with MC Dropout:')

            properties = preprocessed['data_properties']

            # Use the existing sliding window prediction but with MC dropout enabled
            # This ensures we get the same spatial processing as regular predictions
            prediction = self.predict_logits_from_preprocessed_data(data).cpu().detach().numpy()

            # Export segmentation prediction (mean of MC samples is computed inside predict_logits_from_preprocessed_data)
            if ofile is not None:
            ...
            # Export uncertainty map if available (computed inside _internal_maybe_mirror_and_predict)
            if save_uncertainty and uncertainty_output_folder is not None and hasattr(self, 'latest_uncertainty_map') and self.latest_uncertainty_map is not None:
                case_id = os.path.basename(ofile) if ofile else f"case_{len(results)}"
                out_unc_path = os.path.join(uncertainty_output_folder, f"{case_id}_uncertainty.nii.gz")
                save_uncertainty_map_with_correct_shape(
                    self.latest_uncertainty_map.cpu().numpy(),
                    properties,
                    self.plans_manager,
                    self.configuration_manager,
                    out_unc_path
            ...
            results.append({
                ...
                'uncertainty': self.latest_uncertainty_map.cpu().numpy() if hasattr(self, 'latest_uncertainty_map') and self.latest_uncertainty_map is not None else None,
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        def mc_forward_passes(model, x, num_passes=20):
            ...
            for _ in range(num_passes):
                with torch.no_grad():
                    pred = model(x)
                    preds.append(pred.unsqueeze(0))  # Add batch dimension for stacking
            
            return torch.cat(preds, dim=0)  # Shape: (num_passes, batch, channels, ...)
        
        # Perform MC dropout
        mc_preds = mc_forward_passes(self.network, x, num_passes=20)

        # Calculate uncertainty using predictive entropy
        # Convert logits to probabilities
        mc_probs = torch.softmax(mc_preds, dim=2)  # Apply softmax to get probabilities
        
        # Calculate mean probabilities across MC samples
        mean_probs = mc_probs.mean(dim=0)  # Shape: (batch, channels, ...)
        
        # Calculate predictive entropy as uncertainty measure
        # Entropy = -sum(p * log(p)) where p is the mean probability
        eps = 1e-8  # Small epsilon to avoid log(0)
        entropy = -(mean_probs * torch.log(mean_probs + eps)).sum(dim=1, keepdim=True)  # Sum over class dimension
        
        # Normalize entropy to [0, 1] range for better visualization
        # Max entropy for C classes is log(C)
        num_classes = mc_probs.shape[2]
        max_entropy = torch.log(torch.tensor(num_classes, dtype=entropy.dtype, device=entropy.device))
        normalized_entropy = entropy / max_entropy
        
        # Store uncertainty map
        if normalized_entropy.shape[0] == 1 and normalized_entropy.ndim == 5:
            normalized_entropy = normalized_entropy.squeeze(0)
        self.latest_uncertainty_map = normalized_entropy

        # Get mean prediction (in logits)
        prediction = mc_preds.mean(dim=0)
    ...
    def predict_from_files_sequential(self,
        ...
        for li, of, sps in zip(list_of_lists_or_source_folder, output_filename_truncated, seg_from_prev_stage_files):
            ...
            # MC Dropout: collect N samples, aggregate, then compute uncertainty
            mc_samples = []
            num_mc = 20
            self.network.train()
            with torch.no_grad():
                for _ in range(num_mc):
                    logits = self.network(data_torch)
                    mc_samples.append(logits.cpu().numpy())
            mc_samples = np.stack(mc_samples, axis=0)
            mean_logits = np.mean(mc_samples, axis=0)
            probs = torch.softmax(torch.from_numpy(mc_samples), dim=1).numpy()
            mean_probs = np.mean(probs, axis=0)
            eps = 1e-8
            entropy = -np.sum(mean_probs * np.log(mean_probs + eps), axis=0)
            # Use a deepcopy of the original data_properties for both exports
            data_properties_for_export = deepcopy(data_properties)
            if 'path' not in data_properties_for_export or data_properties_for_export['path'] is None:
                data_properties_for_export['path'] = li[0] if isinstance(li, list) and len(li) > 0 else li
                print(f"[DEBUG] Added original image path to properties: {data_properties_for_export['path']}")
            ...
            if of is not None:
                ...
                export_prediction_from_logits(torch.from_numpy(mean_logits), data_properties_for_export, self.configuration_manager, self.plans_manager,
                  self.dataset_json, of, save_probabilities)
            else:
                print(f"✅ Prediction returned for in-memory case (not saved to disk)")
                ret.append(convert_predicted_logits_to_segmentation_with_correct_shape(torch.from_numpy(mean_logits), self.plans_manager,
                     self.configuration_manager, self.label_manager,
                     data_properties_for_export,
                     save_probabilities))
            # Save uncertainty map using the exact same pipeline as predictions
            if save_uncertainty and uncertainty_output_folder is not None:
                case_id = os.path.basename(of) if of else f"case_{len(ret)}"
                unc_output_path = os.path.join(uncertainty_output_folder, f"{case_id}_uncertainty.nii.gz")
                save_uncertainty_map_with_correct_shape(
                    entropy,
                    data_properties_for_export,
                    self.plans_manager,
        ...
        return ret
            ...
            if self.latest_uncertainty_map is not None:
                print("[DEBUG] Saving uncertainty map using exact prediction pipeline")
                ...
                # Use the NEW function that applies the exact same pipeline as predictions
                save_uncertainty_map_with_correct_shape(
                    unc_map,
                    data_properties_for_export,  # Use SAME properties as prediction
                    self.plans_manager,
                    self.configuration_manager,
                    unc_output_path
                )
                print(f"✅ Saved uncertainty map to: {unc_output_path}")
...
if __name__ == '__main__':
    ...
    # Get properties for the case (from the last prediction)
    properties = None
    if hasattr(predictor, 'latest_properties'):
    ...
    if predictor.latest_uncertainty_map is not None and properties is not None:
        unc_map = predictor.latest_uncertainty_map.cpu().numpy()
        ...
        print("No uncertainty map or properties available!")
