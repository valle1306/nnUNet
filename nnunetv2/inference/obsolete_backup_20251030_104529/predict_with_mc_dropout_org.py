...
def enable_mc_dropout(model, dropout_prob=0.05):
    """Enable dropout layers during inference for MC Dropout"""
    for m in model.modules():
        if isinstance(m, torch.nn.Dropout):
            m.p = 0.05  # Explicitly set dropout probability
            m.train()
                                                                                    

def save_uncertainty_map(uncertainty: torch.Tensor, input_image_paths: List[str], output_dir: str):
    """
    Save uncertainty map with proper shape handling for multi-channel input
    """
    os.makedirs(output_dir, exist_ok=True)
    
    # Load the FIRST channel as reference (they should all have same spatial dimensions)
    reference_img = nib.load(input_image_paths[0])  # Just one channel for spatial reference
    reference_shape = reference_img.shape  # (240, 240, 155) - original spatial dimensions
    ...
    print(f"Reference image shape (single channel): {reference_shape}")
    print(f"Uncertainty map shape: {uncertainty_np.shape}")
    
    # Handle different uncertainty map dimensions
    if uncertainty_np.ndim == 5:  # (batch, classes, H, W, D) - MC dropout output
        print(f"5D uncertainty detected: batch={uncertainty_np.shape[0]}, classes={uncertainty_np.shape[1]}")
        # Remove batch dimension and average across classes
        uncertainty_np = np.squeeze(uncertainty_np, axis=0)  # Remove batch dimension: (classes, H, W, D)
        uncertainty_np = np.mean(uncertainty_np, axis=0)     # Average across classes: (H, W, D)
        print(f"After processing 5D: {uncertainty_np.shape}")
    elif uncertainty_np.ndim == 4:  # (classes, H, W, D)
        print(f"4D uncertainty detected. Averaging across {uncertainty_np.shape[0]} classes.")
        uncertainty_np = np.mean(uncertainty_np, axis=0)  # Average across classes: (H, W, D)
        print(f"After processing 4D: {uncertainty_np.shape}")
    elif uncertainty_np.ndim == 3:  # Already (H, W, D)
        print("3D uncertainty detected - already correct shape.")
    else:
        raise ValueError(f"Unexpected uncertainty map dimensions: {uncertainty_np.shape}. Expected 3D, 4D, or 5D.")
    
    # Remove any remaining singleton dimensions and ensure float32
    uncertainty_np = np.squeeze(uncertainty_np).astype(np.float32)
    
    print(f"Final uncertainty shape after squeeze: {uncertainty_np.shape}")
    
    # Ensure we have 3D spatial dimensions
    if uncertainty_np.ndim != 3:
        raise ValueError(f"After processing, uncertainty map should be 3D, got {uncertainty_np.ndim}D with shape {uncertainty_np.shape}")
    
    # Resize uncertainty map to match original spatial dimensions
    if uncertainty_np.shape != reference_shape:
        print(f"Resizing uncertainty map from {uncertainty_np.shape} to {reference_shape}")
        # Calculate zoom factors for each dimension
        zoom_factors = []
        for ref_dim, unc_dim in zip(reference_shape, uncertainty_np.shape):
            zoom_factors.append(ref_dim / unc_dim)
        print(f"Zoom factors applied: {zoom_factors}")
        # Apply zoom with proper error handling
        try:
            print("NaNs before zoom:", np.isnan(uncertainty_np).sum())
            uncertainty_np = zoom(uncertainty_np, zoom_factors, order=1, mode='nearest')
            print(f"Successfully resized to: {uncertainty_np.shape}")
            print("NaNs after zoom:", np.isnan(uncertainty_np).sum())
            # Replace NaNs with zeros
            uncertainty_np = np.nan_to_num(uncertainty_np, nan=0.0)
            print("NaNs after nan_to_num:", np.isnan(uncertainty_np).sum())
        except Exception as e:
            print(f"Error during zoom operation: {e}")
            print(f"Zoom factors: {zoom_factors}")
            print(f"Source shape: {uncertainty_np.shape}")
            print(f"Target shape: {reference_shape}")
            raise
    
    # Verify final shape matches reference
    if uncertainty_np.shape != reference_shape:
        print(f"WARNING: Final uncertainty shape {uncertainty_np.shape} doesn't match reference {reference_shape}")
    
    # Save uncertainty map
    out_path = os.path.join(output_dir, "uncertainty.nii.gz")
    nib.save(nib.Nifti1Image(uncertainty_np, affine, header), out_path)
    print(f"✅ Saved uncertainty map to: {out_path}")
    print(f"Final saved shape: {uncertainty_np.shape}")
...
class nnUNetPredictor(object):
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        def mc_forward_passes(model, x, num_passes=20):
            ...
            enable_mc_dropout(model, dropout_prob=0.05)  # Then enable dropout layers
            preds = []
        ...
        # Store uncertainty map (variance across MC samples)
        self.latest_uncertainty_map = mc_preds.var(dim=0)  # Variance across MC samples
    ...
    def predict_from_files_sequential(self,
        ...
        for li, of, sps in zip(list_of_lists_or_source_folder, output_filename_truncated, seg_from_prev_stage_files):
            ...
            if of is not None:
                ...
                print(f"✅ Saved prediction to: {of}{self.dataset_json['file_ending']}")
            else:
...
if __name__ == '__main__':
    ...
    if 'nnUNet_results' not in os.environ:
        os.environ['nnUNet_results'] = '/scratch/hpl14/nnUNet_results'
    if 'nnUNet_raw' not in os.environ:
        os.environ['nnUNet_raw'] = '/scratch/hpl14/nnUNet_raw'
    
    # Create output directories
    os.makedirs('/home/hpl14/temp/predictions', exist_ok=True)
    os.makedirs('/home/hpl14/MC_predictions/BraTS-PED-00062/', exist_ok=True)
    ...
    predictor.initialize_from_trained_model_folder(
        join(os.environ['nnUNet_results'], 'Dataset999_BraTSPED/nnUNetTrainer__nnUNetPlans__3d_fullres'),
        use_folds=(0,),
...
if predictor.latest_uncertainty_map is not None:
    unc = predictor.latest_uncertainty_map.cpu().numpy()
    print("Uncertainty stats before saving: min=", np.min(unc), "max=", np.max(unc), "mean=", np.mean(unc))
    if np.allclose(unc, 0):
        print("WARNING: Uncertainty map is all zeros. MC Dropout may not be enabled or model lacks dropout layers.")
    save_uncertainty_map(
        predictor.latest_uncertainty_map,
        input_image_paths=[
            "/scratch/hpl14/nnUNet_raw/Dataset999_BraTSPED/imagesTs/BraTS-PED-00062-0000.nii.gz",
            "/scratch/hpl14/nnUNet_raw/Dataset999_BraTSPED/imagesTs/BraTS-PED-00062-0001.nii.gz",
            "/scratch/hpl14/nnUNet_raw/Dataset999_BraTSPED/imagesTs/BraTS-PED-00062-0002.nii.gz",
        ...
        output_dir="/home/hpl14/MC_predictions/BraTS-PED-00062/"
    )
else:
