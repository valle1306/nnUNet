...
def save_uncertainty_map_remap(uncertainty: np.ndarray, properties: dict, plans_manager, configuration_manager, output_path: str, label_manager=None):
    ...
    # Ensure properties['spacing'] is a list of floats for downstream code
    if 'spacing' in properties:
        # Convert np.float64 to float if needed
        properties['spacing'] = [float(x) for x in properties['spacing']]
    elif original_spacing is not None:
        properties['spacing'] = [float(x) for x in original_spacing]
    """
